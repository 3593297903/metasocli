...
export async function safePath(input: string): Promise<string> {
  ...
  for (const part of full.slice(current.length).split(path.sep).filter(Boolean)) {
    ...
    let info;
    try { info = await lstat(current); } catch (e) { if (isCode(e, 'ENOENT')) continue; throw e; }
    if (info.isSymbolicLink()) fail('LINK_PATH', 'Symbolic links and directory junctions are not writable project paths.');
    const real = await realpath(current);
    if (key(real) !== key(current)) fail('LINK_PATH', 'Canonical path differs from the requested path.');
    if (info.isFile() && info.nlink !== 1) fail('LINK_PATH', 'Managed files must not be hardlinked.');
