...
import { inspectAudio, MAX_AUDIO_BYTES } from '../assets/audio.js';
...
const Content = z.discriminatedUnion('type', [
  ...
  z.object({ type: z.literal('audio_url'), role: z.literal('reference_audio'), audio_url: z.object({ url: z.string().min(1) }).strict() }).strict(),
]);
export const RequestSchema = z.object({
  model: z.literal('MiniMax-H3'), content: z.array(Content).min(1).max(11),
...
export function validateRequest(value: unknown): H3Request {
  ...
  const audios = request.content.filter(i => i.type === 'audio_url');
  if (images.length > 9 || audios.length > 1) fail('H3_CONTENT', 'This client supports up to nine images and one narration reference.');
  ...
  if (first.length && (first.length !== 1 || images.length !== 1 || audios.length || request.ratio !== 'adaptive')) fail('H3_MODE', 'A single first frame requires adaptive ratio and cannot mix with references.');
  if (!images.length && !audios.length && request.ratio === 'adaptive') fail('H3_RATIO', 'Text-to-video requires an explicit aspect ratio.');
  ...
  for (const audio of audios) {
    const url = audio.audio_url.url;
    if (url.startsWith('data:')) {
      const match = /^data:(audio\/(?:mpeg|wav));base64,([A-Za-z0-9+/]+={0,2})$/u.exec(url);
      if (!match) fail('H3_AUDIO', 'Audio data URL is malformed.');
      const bytes = Buffer.from(match[2]!, 'base64');
      if (bytes.toString('base64') !== match[2] || inspectAudio(bytes).mime !== match[1]) fail('H3_AUDIO', 'Audio data URL content does not match its MIME.');
    } else publicHttps(url);
  }
...
export async function buildRequest(root: string, story: Story, segment: StorySegment) {
  ...
  const assets: GenerationPlan['segments'][number]['assets'] = [];
  for (const ref of segment.references) {
    ...
    if (!('width' in media)) fail('ASSET_ROLE', 'Audio cannot be sent as an image reference.');
  ...
  if (segment.narration) {
    const asset = story.assets.find(a => a.recipe.assetId === segment.narration!.assetId);
    if (!asset?.media) fail('MISSING_ASSET', `Missing narration audio for ${segment.id}; restore the reference before planning.`);
    const media = asset.media;
    if (asset.recipe.kind !== 'narration' || !('duration' in media)) fail('ASSET_ROLE', 'Narration requires a registered audio asset.');
    const bytes = await readStable(await projectPath(root, media.path), MAX_AUDIO_BYTES);
    const audio = inspectAudio(bytes);
    if (sha256Hex(bytes) !== media.sha256 || bytes.length !== media.bytes || audio.mime !== media.mime || audio.duration !== media.duration || audio.sampleRate !== media.sampleRate || audio.channels !== media.channels) fail('ASSET_CHANGED', `Narration audio changed for ${segment.id}; re-register and replan.`);
    const url = media.transport === 'url' ? publicHttps(media.url!) : `data:${media.mime};base64,${bytes.toString('base64')}`;
    content.push({ type: 'audio_url', role: 'reference_audio', audio_url: { url } });
    assets.push({ assetId: asset.recipe.assetId, role: 'reference_audio', sha256: media.sha256, recipeHash: asset.recipeHash, transport: media.transport, duration: media.duration });
  }
  ...
  return { request, summary: {
    ...
    ...(segment.targetDurationSeconds !== undefined ? { targetDurationSeconds: segment.targetDurationSeconds } : {}),
    effectiveRatio: request.ratio, contextIr: request.context_ir_enabled, watermark: request.aigc_watermark, assets,
    ...(segment.narration !== undefined ? { narration: segment.narration } : {}),
  } };
}
