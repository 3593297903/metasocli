...
export async function registerAsset(root: string, id: string, input: { file?: string; url?: string; provenance?: 'user' | 'imagegen' }, fetcher?: Fetch) {
  parse(Id, id);
  input = parse(z.object({ file: z.string().min(1).optional(), url: z.string().min(1).optional(), provenance: z.enum(['user', 'imagegen']).optional() }).strict(), input);
  ...
  return withProjectLock(root, async () => {
    ...
    if (canonicalSha256(asset.media) === canonicalSha256(media)) {
      ...
      try { if (sha256Hex(await readStable(target, MAX_IMAGE_BYTES)) === hash) return asset; } catch { /* Restore a missing or damaged managed copy. */ }
    }
