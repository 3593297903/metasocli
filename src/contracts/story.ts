...
export const Recipe = z.object({
  assetId: Id, kind: z.enum(['character', 'scene', 'prop', 'first_frame']),
...
export const Segment = z.object({
  ...
  duration: z.number().int().min(4).max(15), prompt: z.string().min(1), promptHash: Hash,
...
export const ImportDraft = z.object({
  ...
  segments: z.array(z.object({
    id: Id, start: z.number().int().nonnegative(), end: z.number().int().positive(),
    duration: z.number().int().min(4).max(15), prompt: z.string().min(1).optional(),
